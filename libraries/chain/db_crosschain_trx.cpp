...
// 					FC_ASSERT(trx_itr == trx_db.end(), "This sign tex is exist");
		...
		void database::create_result_transaction(miner_id_type miner, fc::ecc::private_key pk) {
			try {
				...
				auto check_point_2 = get_index_type<crosschain_trx_index>().indices().get<by_transaction_stata>().equal_range(withdraw_transaction_confirm);
				...
				for (auto & trxs : uncombine_trxs_counts) {
					...
					for (auto & sign_trx_id : trxs.second) {
						...
						if (sign_senator == guard_member_id_type()) {
							sign_senator = with_sign_op.sign_guard;
						}
					}
					if (transaction_err) {
						continue;

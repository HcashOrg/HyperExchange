...
namespace uvm
{
	...
	{
		_scope = std::make_shared<uvm::lua::lib::GluaStateScope>(use_contract);
	}
	...
	void GluaContractEngine::set_state_pointer_value(std::string name, void *addr)
	{
		...
		uvm::lua::lib::set_lua_state_value(_scope->L(), name.c_str(), statevalue, GluaStateValueType::LUA_STATE_VALUE_POINTER);
	}

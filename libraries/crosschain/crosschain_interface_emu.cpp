...
namespace graphene {
	namespace crosschain {
		...
		bool crosschain_interface_emu::create_signature(const std::string &account, const std::string &content, std::string &signature)
		{
			return false;
		}
		...
		std::vector<fc::variant_object> crosschain_interface_emu::transaction_history(std::string symbol,std::string &user_account, uint32_t start_block, uint32_t limit, uint32_t& end_block_num)
		{
			...
			const auto &itr = blocks.lower_bound(start_block, comp_block_num());
			std::vector<fc::variant_object> ret;

...
namespace graphene {
	namespace crosschain {
		...
		fc::variant_object crosschain_interface_btc::transaction_query(vector<std::string>& trx_ids)
		{
			try {
				std::ostringstream req_body;
				req_body << "{ \"jsonrpc\": \"2.0\", \
		...
		std::vector<fc::variant_object> crosschain_interface_btc::transaction_history_all(std::vector<fc::mutable_variant_object> mul_param_obj){
			std::vector<fc::variant_object> return_value;
			std::ostringstream req_body;
			req_body << "{ \"jsonrpc\": \"2.0\", \
                \"id\" : \"45\", \

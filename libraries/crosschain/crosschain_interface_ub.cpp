...
namespace graphene {
	namespace crosschain {
		...
		std::string crosschain_interface_ub::create_normal_account(std::string account_name, const fc::optional<fc::ecc::private_key> key/*=fc::optional<fc::ecc::private_key>()*/)
		{
			...
			//	\"params\" : {\"chainId\":\"ub\" ,\"addr\": \"" << ptr->get_address() << "\"}}";
		...
		fc::variant_object crosschain_interface_ub::create_multisig_transaction(const std::string &from_account, const std::map<std::string, std::string> dest_info, const std::string &symbol, const std::string &memo)
		{
			std::ostringstream req_body;
			req_body << "{ \"jsonrpc\": \"2.0\", \
                \"id\" : \"45\", \
		...
		bool crosschain_interface_ub::create_signature(graphene::privatekey_management::crosschain_privatekey_base*& sign_key, const std::string &content, std::string &signature)
		{

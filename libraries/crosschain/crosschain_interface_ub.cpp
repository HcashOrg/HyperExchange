...
namespace graphene {
	namespace crosschain {
		...
		fc::variant_object crosschain_interface_ub::transfer(std::string &from_account, std::string &to_account, uint64_t amount, std::string &symbol, std::string &memo, bool broadcast /*= true*/)
		{
			return fc::variant_object();
		}

		fc::variant_object crosschain_interface_ub::create_multisig_transaction(std::string &from_account, const std::map<std::string, std::string> dest_info, std::string &symbol, std::string &memo, const std::string& prk)
		{
			std::ostringstream req_body;
			req_body << "{ \"jsonrpc\": \"2.0\", \
                \"id\" : \"45\", \
		...
		std::string crosschain_interface_ub::sign_multisig_transaction(fc::variant_object trx, std::string &sign_account, const std::string& redeemScript, bool broadcast /*= true*/)
		{
		...
		std::vector<fc::variant_object> crosschain_interface_ub::transaction_history(std::string symbol, std::string &user_account, uint32_t start_block, uint32_t limit, uint32_t& end_block_num)
		{
